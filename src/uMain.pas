...
begin
  ...
  if dbSQL.IsConnected
    then begin
      ...
      ShowMessage('N�o foi poss�vel conectar ao servidor. . .');
    end;
end;
...
begin
  if MessageDlg(
       Format('%d ser� gravado como o novo valor de visualiza��es.'#10'Confirma?', [NewValue]),
       mtConfirmation,
       [mbYes, mbNo],
       0,
       mbYes
...
begin
  try
    NewValue := StrToInt(
      InputBox(
        'Substituir n�mero de visualiza��es',
    ...
    on E: EConvertError do
      begin
        ShowMessage('O valor deve ser num�rico.');
